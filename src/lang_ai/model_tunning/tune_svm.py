"""
Hyperparameter tuning for Support Vector Machine (SVM) using Optuna.
...
class SVMTuner:
    """
    Tunes SVM hyperparameters using Optuna.
    ...
    def __init__(
        ...
        data_path: Path,
        output_dir: Path,
        study_name: str = "political_leaning_svm_f1",
        n_trials: int = 100,
    ...
    def objective(self, trial: Trial) -> float:
        ...
        """
        # Regularization parameter (inverse strength)
        ...
        # Dual formulation (recommended when n_samples > n_features)
        # For text classification, n_features is typically large, so we try both
    ...
    def train_best_model(self) -> LinearSVC:
        """
        ...
        self.best_model = LinearSVC(
            ...
            dual=p["dual"],
            tol=p["tol"],
            max_iter=p["max_iter"] * 2,  # More iterations for final model
            random_state=self.random_state,
        )
        ...
        logger.info(
            f"Best params: C={p['C']:.4f}, loss={p['loss']}, "
...
def main():
    parser = argparse.ArgumentParser(
        description="Tune SVM for Political Leaning Classification (Macro F1)"
    )
    parser.add_argument(
        "--data",
        type=str,
    ...
    parser.add_argument(
        "--output",
        type=str,
        default="outputs/svm_tuning_f1",
        help="Output directory for results",
    )
    parser.add_argument(
        "--study-name",
        type=str,
        default="political_leaning_svm_f1",
