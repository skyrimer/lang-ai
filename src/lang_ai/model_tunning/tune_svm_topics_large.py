"""
Hyperparameter tuning for Support Vector Machine (SVM) using Optuna with Cross-Validation.

Optimizes a linear SVM classifier for political leaning prediction
...
class SVMTuner:
    """
    Tunes SVM hyperparameters using Optuna with Cross-Validation.
    """
    ...
    def __init__(
        ...
        data_path: Path,
        output_dir: Path,
        study_name: str = "political_leaning_svm_cv",
        n_trials: int = 100,
        n_folds: int = 5,
        random_state: int = 42,
        n_jobs: int = 4,
    ...
    def objective(self, trial: Trial) -> float:
        """
        Performs Stratified Group K-Fold CV.
        ...
        # Handle dual automatically based on loss/n_samples logic or force auto
        dual = "auto"
        ...
        for fold_idx, (train_idx, val_idx) in enumerate(cv.split(self.X, self.y, groups=self.groups)):
            ...
            # model.coef_ shape is (n_classes, n_features) for multiclass
            # or (1, n_features) for binary.
            top_feats_indices = {}
            if model.coef_.ndim == 1:
                # Binary case: Class 1 is positive, Class 0 is negative
                # We treat them as two classes for consistency
    ...
    def train_final_model(self) -> None:
        ...
        final_params["dual"] = "auto"
        # Increase max_iter for final build to ensure convergence
        final_params["max_iter"] = final_params["max_iter"] * 2

        self.best_model = LinearSVC(**final_params)
...
def main():
    parser = argparse.ArgumentParser(description="Tune SVM with CV and Stability Analysis")
    parser.add_argument("--data", type=str, default="preprocessed_data/preprocessed_data.csv")
    parser.add_argument("--output", type=str, default="outputs/svm_cv_stability")
    parser.add_argument("--study-name", type=str, default="svm_cv_stability")
